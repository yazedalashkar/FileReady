...
export default function SmartRequirements({
  ...
  // Evaluate requirements whenever inspection or requirements change
  ...
  const handleMaxNumericChange = (val, unit) => {
    setActivePresetId('custom');
  ...
  return (
    <div className="bg-white dark:bg-slate-900/80 border border-slate-200/90 dark:border-slate-800 rounded-3xl p-5 sm:p-6 shadow-xs space-y-4 transition-colors animate-in fade-in duration-200">
      ...
      <div className="space-y-2">
        <div className="overflow-hidden rounded-2xl border border-slate-100 dark:border-slate-800/80 bg-slate-50/60 dark:bg-slate-950/60">
          <table
          ...
          >
            <thead>
              <tr className="border-b border-slate-200/80 dark:border-slate-800 text-[10px] uppercase font-bold text-slate-400 dark:text-slate-500 tracking-wider">
                ...
                <th className="py-2.5 px-3 text-start">{t.reqRequired}</th>
                <th className="py-2.5 px-3 text-start">{t.reqActual}</th>
                <th className="py-2.5 px-3 text-end">{t.reqStatus}</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800/60 font-medium">
              {evaluation.rules.map((rule) => {
                const reqLabel = rule.isTranslationKey ? t[rule.requiredText] : rule.requiredText;
                ...
                return (
                  ...
                  >
                    <td className="py-2.5 px-3 font-semibold text-[#0B1220] dark:text-white">
                      {t[rule.labelKey] || rule.labelKey}
                    </td>
                    <td className="py-2.5 px-3 text-slate-500 dark:text-slate-400">
                      {reqLabel}
                    </td>
                    <td className="py-2.5 px-3 text-[#0B1220] dark:text-slate-200">
                      {actLabel}
                    </td>
                    <td className="py-2.5 px-3 text-end">
                      {rule.status === 'PASSED' && (
                        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-lg text-[10px] sm:text-[11px] font-bold bg-emerald-100 dark:bg-emerald-950/60 text-emerald-700 dark:text-emerald-300">
                          ✓ {t.reqRulePassed}
                        </span>
                      )}
                      {rule.status === 'FAILED' && (
                        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-lg text-[10px] sm:text-[11px] font-bold bg-rose-100 dark:bg-rose-950/60 text-rose-700 dark:text-rose-300">
                          ✗ {t.reqRuleFailed}
                        </span>
                      )}
                      {rule.status === 'UNDETERMINED' && (
                        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-lg text-[10px] sm:text-[11px] font-bold bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400">
      ...
      <div className="pt-1">
        {evaluation.isCompliant ? (
          <div className="p-3.5 rounded-2xl bg-emerald-50/90 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-800/60 text-emerald-950 dark:text-emerald-200 flex items-start gap-2.5">
          ...
          <div className="space-y-3">
            <div className="p-3.5 rounded-2xl bg-amber-50/90 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800/60 text-amber-950 dark:text-amber-200 flex items-start gap-2.5">
              <span className="text-base shrink-0">⚠</span>
              <div className="space-y-0.5">
            ...
            <button
              ...
              className="w-full h-11 sm:h-12 rounded-xl font-bold text-xs sm:text-sm bg-blue-600 hover:bg-blue-700 active:bg-blue-800 text-white shadow-sm shadow-blue-500/20 transition-all flex items-center justify-center gap-2 cursor-pointer"
